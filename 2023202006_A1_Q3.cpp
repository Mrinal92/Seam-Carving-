...
int main() {
	...
	while(RemoveW--) {
		...
		int Min = 0;
		for(int i = 0; i < w; i++) {
			if(SeamCarv[h - 1][i] < SeamCarv[h - 1][Min]) Min = i;
		}
		// backtracking
		int SeamArr[h];
		SeamArr[h - 1] = Min;
		...
		for(int i = h - 2; i > -1; i--) {
			...
			int right = posval + 1;
			int top = posval;
			if(posval <= 0) left = w - 1;
			if(posval >= w - 1) right = 0;
			int topleft = SeamCarv[i][left];
			int topcenter = SeamCarv[i][top];
			int topright = SeamCarv[i][right];
			if(topleft == miin(SeamCarv[i][left], SeamCarv[i][top], SeamCarv[i][right])) SeamArr[i] = posval - 1;
			else if(topcenter == miin(SeamCarv[i][left], SeamCarv[i][top], SeamCarv[i][right])) SeamArr[i] = posval;
			else if(topright == miin(SeamCarv[i][left], SeamCarv[i][top], SeamCarv[i][right])) SeamArr[i] = posval + 1;
			else;
		}
		for(int i = 0; i < h; i++) {
